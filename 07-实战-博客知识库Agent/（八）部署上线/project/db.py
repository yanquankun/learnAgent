...
DB_PATH = config.DATA_DIR / "blog_agent.db"
...
# ============================================================
# qa_logs 问答记录（第七章：评估与排错的原材料）
# ============================================================

def save_qa_log(
    trace_id: str, session_id: str, question: str, answer: str,
    category: str, confidence: float, source_cnt: int, latency_ms: int,
) -> None:
    conn = get_db()
    with conn:
        conn.execute(
            "INSERT INTO qa_logs (trace_id, session_id, question, answer, category,"
            " confidence, source_cnt, latency_ms, created_at) VALUES (?,?,?,?,?,?,?,?,?)",
            (trace_id, session_id, question, answer, category, confidence,
             source_cnt, latency_ms, datetime.now().isoformat(timespec="seconds")),
        )
    conn.close()


# ============================================================
# memory_events 用户记忆审计账（08 模块三章接入）
# ============================================================
